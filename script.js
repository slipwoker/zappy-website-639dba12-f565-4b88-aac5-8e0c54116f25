document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    const VALIDATORS = {
      ...
      tel: (value) => value.trim() === '' || /^[\d\s\+\-\(\)]{7,20}$/.test(value.trim()),
      minlength: (value, param) => value.trim().length >= parseInt(param, 10)
    };

    function getErrorMessage(field, type) {
      const label =
        field.getAttribute('aria-label') ||
        (field.id && contactForm.querySelector('label[for="' + field.id + '"]')
          ? contactForm.querySelector('label[for="' + field.id + '"]').textContent.trim()
          : null) ||
        field.name ||
        'This field';
      ...
      return messages[type] || label + ' is invalid.';
    }
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      if (!field || field.type === 'submit' || field.type === 'button') return;
      validateField(field);
    }, true);
  ...
  const OBSERVED_SELECTOR = '.fade-in, .animate-on-scroll, [data-animate]';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '639dba12-f565-4b88-aac5-8e0c54116f25',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '639dba12-f565-4b88-aac5-8e0c54116f25';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '639dba12-f565-4b88-aac5-8e0c54116f25';
